...
Rules:
1. Generate exactly ONE question based solely on the provided text.
2. Attempt to generate a concise, factual answer using only the text chunk.
3. If the answer would involve or reflect any of the following:
   - Hate speech, abusive or offensive language
   - Discrimination (race, gender, religion, nationality, disability, or orientation)
   - Harassment, explicit content, or harmful intent
   Then:
     - Still include the generated **question** if it is safe.
     - Do NOT generate an actual answer.
     - Instead, return a **"warning"** field with a brief explanation of why the answer is withheld.

4. The question and answer must be in the SAME language as the text chunk.
5. Respond ONLY with a valid JSON object with the following structure:
    - Always include the key `"question"`.
    - Include either:
        - `"answer"`: if safe
        - or `"warning"`: if the answer would be inappropriate
